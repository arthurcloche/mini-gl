...
class miniGL {
  ...
  updateCanvasTexture(texture, canvas) {
    if (!texture || !canvas) return;

    // Bind the texture
    this.gl.bindTexture(this.gl.TEXTURE_2D, texture);

    // Update the texture data from the canvas
    this.gl.texImage2D(
      this.gl.TEXTURE_2D,
      0,
      this.gl.RGBA,
  ...
  /**
   * Creates a texture from an HTML canvas element
   * @param {HTMLCanvasElement} canvas - The canvas to create a texture from
   * @param {Function} [draw] - Optional function to draw on the canvas (ctx, width, height, gl) => void
   * @returns {Object} An object with the texture and update/destroy methods
   */
  createCanvasTexture(canvas, draw) {
    const gl = this.gl;
    const texture = gl.createTexture();
    ...
    // Set texture parameters - no mipmaps for dynamic textures
    ...
    return {
      texture,
      update() {
        // Safety: make sure we're not bound to a framebuffer
        const currentFramebuffer = gl.getParameter(gl.FRAMEBUFFER_BINDING);
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);

        // If we have a draw function, call it
        ...
        // Unbind the texture to prevent feedback loops
        gl.bindTexture(gl.TEXTURE_2D, null);
   ...
   * @param {HTMLImageElement} image - The image to create a texture from
  ...
  createImageTexture(image) {
    ...
    // Generate mipmaps for static images
