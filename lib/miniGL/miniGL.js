...
class miniGL {
  constructor(id, options = {}) {
    ...
    this.ignoreMiniNodes = options.ignoreMiniNodes === true; // Default to false (load miniNodes)
    ...
    // Start loading miniNodes if not ignored (async)
    if (!this.ignoreMiniNodes) {
  ...
  async loadMiniNodes() {
    ...
    try {
      // Import individual utility nodes and register them
      const luminanceModule = await import(
        "./miniNodes/effects/luminanceNode.js"
      );
      luminanceModule.registerLuminanceNode(this);

      const grayscaleModule = await import(
        "./miniNodes/effects/grayscaleNode.js"
      );
      grayscaleModule.registerGrayscaleNode(this);

      const mixModule = await import("./miniNodes/effects/mixNode.js");
      mixModule.registerMixNode(this);

      const gaussianBlurModule = await import(
        "./miniNodes/effects/gaussianBlurNode.js"
      );
      gaussianBlurModule.registerGaussianBlurNode(this);

      const brightnessContrastModule = await import(
        "./miniNodes/effects/brightnessContrastNode.js"
      );
      brightnessContrastModule.registerBrightnessContrastNode(this);

      const saturationModule = await import(
        "./miniNodes/effects/saturationNode.js"
      );
      saturationModule.registerSaturationNode(this);

      // Import effect nodes and register them
      const bloomModule = await import("./miniNodes/effects/bloomNode.js");
      bloomModule.registerBloomNode(this);

      this._miniNodesLoaded = true;
      console.log("MiniNodes loaded successfully");
    } catch (error) {
      console.warn("Failed to load miniNodes:", error);
      this.ignoreMiniNodes = true;
    }
