...
class miniGL {
  constructor(target, options = {}) {
    this.options = {
      ...
      fps: options.fps !== undefined ? options.fps : 60, // Default to 60fps
      unlimitedFps: options.unlimitedFps || false, // Allow unlimited fps override
  ...
  setFps(fps) {
    this.targetFps = fps;
    this.frameInterval = fps <= 0 ? 0 : 1000 / fps;
    return this;
  }
  ...
  render(timestamp = performance.now()) {
    ...
    if (!this.outputNode) {
      // Schedule next frame even if no output
      this._animationId = requestAnimationFrame((t) => this.render(t));
      return;
    }
