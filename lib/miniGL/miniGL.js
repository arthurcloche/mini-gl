...
class GroupNode extends TextureNode {
  constructor(gl, options = {}) {
    ...
    this._transparentBlack = gl.TransparentPixel;
...
class miniGL {
  constructor(id, options = {}) {
    ...
    // MiniNodes configuration
    this.enableMiniNodes = options.enableMiniNodes !== false; // Default to true
    this._miniNodesLoaded = false;
    this._miniNodesPromise = null; // Promise for async loading
    ...
    // Start loading miniNodes if enabled (async)
    if (this.enableMiniNodes) {
      this._miniNodesPromise = this.loadMiniNodes();
    } else {
      this._miniNodesPromise = Promise.resolve(this);
    }
  }
  ...
  // Public method to wait for miniNodes to be ready
  async ready() {
    if (this._miniNodesPromise) {
      await this._miniNodesPromise;
    }
    return this;
  }

  // Load miniNodes functionality
  async loadMiniNodes() {
    if (this._miniNodesLoaded) return this;

    try {
      // Import utility nodes
      const utilsModule = await import("./miniNodes/utils.js");
      const {
        LuminanceNode,
        GrayscaleNode,
        MixNode,
      ...
      // Import effect nodes
      const effectsModule = await import("./miniNodes/effects/bloomNode.js");
      const { BloomNode } = effectsModule;

      // Add factory methods for utility nodes
      this.luminance = (threshold = 0.5, knee = 0.1, options = {}) => {
        const node = new LuminanceNode(this.gl, threshold, knee, options);
        return this.addNode(node);
      };

      this.grayscale = (strength = 1.0, options = {}) => {
        const node = new GrayscaleNode(this.gl, strength, options);
        return this.addNode(node);
      };

      this.mix = (mixFactor = 0.5, options = {}) => {
        const node = new MixNode(this.gl, mixFactor, options);
        return this.addNode(node);
      };
      ...
      this.gaussianBlur = (
        ...
        const node = new GaussianBlurNode(this.gl, radius, direction, options);
        return this.addNode(node);
      };
      ...
      this.brightnessContrast = (
        ...
        const node = new BrightnessContrastNode(
          this.gl,
          brightness,
          contrast,
          options
      ...
      this.saturation = (saturation = 1.0, options = {}) => {
        const node = new SaturationNode(this.gl, saturation, options);
        return this.addNode(node);
      };

      // Add factory methods for effect nodes
      this.bloom = (
        threshold = 0.8,
        knee = 0.1,
        intensity = 1.0,
      ...
      this.gaussianBlurH = (radius = 1.0, options = {}) => {
        return this.gaussianBlur(radius, [1.0, 0.0], {
          name: "GaussianBlurH",
          ...options,
        });
      };

      this.gaussianBlurV = (radius = 1.0, options = {}) => {
        return this.gaussianBlur(radius, [0.0, 1.0], {
          name: "GaussianBlurV",
          ...options,
        });
      };
      ...
      this.gaussianBlur2Pass = (radius = 1.0, options = {}) => {
        const blurH = this.gaussianBlurH(radius, {
          name: `${options.name || "GaussianBlur2Pass"}_H`,
        });
        const blurV = this.gaussianBlurV(radius, {
      ...
      this._miniNodesLoaded = true;
      console.log("MiniNodes loaded successfully");
    } catch (error) {
      console.warn("Failed to load miniNodes:", error);
      this.enableMiniNodes = false;
    }
