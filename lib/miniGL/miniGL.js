...
class miniGL {
  constructor(target, options = {}) {
    ...
    this.canvas = this._setupCanvas(target);
  ...
  _setupCanvas(target) {
    ...
    switch (tagName) {
      case "canvas":
        ...
        if (!canvas.id) {
          canvas.id = this.options.id || `minigl-canvas-${Date.now()}`;
        }
        break;
      ...
      case "main":
        ...
        this._applyCanvasOptions(canvas);

        // Size canvas to container
        const rect = element.getBoundingClientRect();
        if (!this.options.width) canvas.width = rect.width || 512;
        if (!this.options.height) canvas.height = rect.height || 512;
      ...
      case "img":
        ...
        canvas.width =
          this.options.width || element.naturalWidth || element.width || 512;
        canvas.height =
          this.options.height || element.naturalHeight || element.height || 512;

        // Replace image with canvas
        element.parentNode.replaceChild(canvas, element);

        // Create image texture node for the original image
        setTimeout(() => {
          if (element.src) {
            this.sourceImageNode = this.image(element.src, {
              width: canvas.width,
      ...
      case "video":
        ...
        canvas.width =
          this.options.width || element.videoWidth || element.width || 512;
        canvas.height =
          this.options.height || element.videoHeight || element.height || 512;

        // Replace video with canvas
        element.parentNode.replaceChild(canvas, element);

        // Create video texture node for the original video
        setTimeout(() => {
          if (element.src) {
            this.sourceVideoNode = this.video(element.src, {
              width: canvas.width,
              height: canvas.height,
              name: "Source Video",
            });
      ...
      default:
        ...
        if (!this.options.width) canvas.width = elementRect.width || 512;
        if (!this.options.height) canvas.height = elementRect.height || 512;

        element.appendChild(canvas);
        break;
    }

    // Apply final sizing and styling
    this._applyCanvasOptions(canvas);
    this._setupResponsiveResize(canvas);
  ...
  _applyCanvasOptions(canvas) {
    ...
    // Default styling for better responsiveness
  ...
  _setupResponsiveResize(canvas) {
    // Set up responsive resize unless explicit dimensions are provided
    if (!this.options.width || !this.options.height) {
      this._resizeHandler = () => {
      ...
      // Add to existing resize listener or create new one
