...
class miniGL {
  ...
  _setupCanvas(target) {
    ...
    switch (tagName) {
      ...
      case "article":
      case "main":
        canvas = this._createResponsiveCanvas();
      ...
      case "video":
        // Create wrapper div to replace the video
        const videoSrc = element.src || element.children[0]?.src;
        const videoWrapper = document.createElement("div");

        // Copy all computed styles from video to wrapper to maintain exact layout
        ...
        // Replace video with wrapper in DOM
        element.parentNode.replaceChild(videoWrapper, element);

        // Hide original video and add it to wrapper (preserves it for reference)
        element.style.display = "none";
        ...
        // Store video info for later loader node creation
        if (videoSrc) {
          this._pendingLoaderNode = {
            type: "video",
            src: videoSrc,
  ...
  _createResponsiveCanvas() {
    ...
    // Make canvas responsive to parent by default
  ...
  _setupResponsiveResize(canvas) {
    // Always set up responsive resize for the canvas to match its parent
    this._resizeHandler = () => {
      ...
      // Update canvas resolution based on display size and DPR
      ...
      if (displayWidth > 0 && displayHeight > 0) {
        ...
        // Keep the canvas styled to match parent size
        canvas.style.width = displayWidth + "px";
        canvas.style.height = displayHeight + "px";

        // Only call this.resize() if miniGL is fully initialized
