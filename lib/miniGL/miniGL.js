...
export class CanvasTextureNode extends TextureNode {
  ...
  process(time) {
    ...
    // Safety check for drawCallback
    if (!this.drawCallback || typeof this.drawCallback !== "function") {
      // Try to recover from _originalDrawCallback if available
      if (this._originalDrawCallback && typeof this._originalDrawCallback === "function") {
        this.drawCallback = this._originalDrawCallback;
      } else {
        // Provide a fallback to prevent crashes
        this.drawCallback = (ctx, w, h) => {
          ctx.fillStyle = "#333";
    ...
    // Call the drawing function with error handling
    try {
      this.drawCallback(this.context, this.width, this.height);
    } catch (error) {
      console.error("Error in canvas drawCallback:", error);
      // Draw error state
    ...
    // Flip Y to match WebGL texture coordinate system
  ...
  // Method to update the draw callback
  updateDrawCallback(drawCallback) {
    if (!drawCallback) {
      return this;
    }
...
class ImageTextureNode extends TextureNode {
  ...
  load(url) {
    ...
    // Check if this is a same-origin request or if we should use CORS
    const isSameOrigin = this.isSameOrigin(url);

    // Always set crossOrigin for external URLs to avoid tainted canvas errors
    // If the server doesn't support CORS, the image will fail to load (which is better than a tainted canvas)
    if (!isSameOrigin) {
  ...
  supportsCORS(url) {
    // List of domains known to support CORS
    // Note: Unsplash domains removed - they no longer support CORS as of 2024
  ...
  process(time) {
    ...
    for (const [inputName, connection] of this.inputs) {
      ...
      if (output?.texture) {
        ...
        console.warn(
          `Input ${inputName} has no texture output from node`,
          connection.node,
          `Output value:`, output
        );
      }
    ...
    // Handle Blend nodes that might be missing inputs
    if (this.name && this.name.includes("Blend")) {
      // If this is a blend node and we don't have both inputs, provide defaults
      if (!inputTextures.glBase || !inputTextures.glBlend) {
        const defaultTextures = this.minigl.getDefaultTextures();
        if (!inputTextures.glBase) {
          inputTextures.glBase = { texture: defaultTextures.black };
          inputTextures.glBaseSize = { x: 1, y: 1 };
        }
        if (!inputTextures.glBlend) {
          inputTextures.glBlend = { texture: defaultTextures.transparent };
          inputTextures.glBlendSize = { x: 1, y: 1 };
        }
      }
    }
...
class miniGL {
  ...
  _render(timestamp = performance.now()) {
    ...
    if (this._updateCallback) {
      this._updateCallback(currentTime);
    }
  ...
  // Helper to create a default texture (1x1 pixel with specified color)
  createDefaultTexture(r = 0, g = 0, b = 0, a = 255) {
    const gl = this.gl;
  ...
  // Get or create default textures
  getDefaultTextures() {
    if (!this._defaultTextures) {
      this._defaultTextures = {
        black: this.createDefaultTexture(0, 0, 0, 255),
        white: this.createDefaultTexture(255, 255, 255, 255),
        transparent: this.createDefaultTexture(0, 0, 0, 0),
      };
    }
    return this._defaultTextures;
  }

  // Create a simple blend shader node - just a shader with 2 texture inputs
  createBlendNode(uniforms = {}, nodeOptions = {}) {
    // Simple all-in-one blend shader
    const fragmentShader = `#version 300 es
    precision highp float;
    ...
    uniform float glBlendMode; // 0=screen, 1=multiply, 2=overlay
    
    in vec2 vTexCoord;
    out vec4 fragColor;
    
    void main() {
      vec4 base = texture(glBase, vTexCoord);
      vec4 blend = texture(glBlend, vTexCoord);
      ...
      if (mode == 1) {
        ...
        // Screen (default, mode == 0)
        result = vec4(1.0) - ((vec4(1.0) - base) * (vec4(1.0) - blend));
      }
      
      fragColor = result;
    }`;

    // Just create a shader node
    return this.shader(
      fragmentShader,
      uniforms,
      { name: nodeOptions.name || "Blend", ...nodeOptions }
    );
  }
