...
// --- ParticleNode: Flexible instanced particle framework ---
class ParticleNode extends TextureNode {
  constructor(gl, options = {}) {
    ...
    this.simulationShader =
      options.simulationShader || this.getDefaultSimulationShader();
    ...
    this.uniforms = {
      // Default particle simulation parameters
      mouseForce: 0.002,
      mouseRadius: 0.1,
      springForce: 0.02,
      damping: 0.95,
  ...
  createSimulationNode() {
    this.simulationNode = new FeedbackNode(this.gl, {
    ...
    this.simulationNode.minigl = this.minigl;
  }
  ...
  process(time) {
    ...
    // Get global uniforms for the simulation
    ...
    // Merge global uniforms with user uniforms for simulation
    this.simulationNode.uniforms = {
      ...globalUniforms,
      ...this.uniforms,
    };

    this.simulationNode.process(time);
    ...
    // Bind particle data texture
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, this.simulationNode.output().texture);
    gl.uniform1i(
      gl.getUniformLocation(this.particleProgram, "particleData"),
      0
    );

    // Set other uniforms
    gl.uniform2f(
      gl.getUniformLocation(this.particleProgram, "glResolution"),
      this.width,
      this.height
    );
    gl.uniform1f(
      gl.getUniformLocation(this.particleProgram, "particleSize"),
      this.particleSize
    );
    gl.uniform1f(
      gl.getUniformLocation(this.particleProgram, "glRatio"),
      renderUniforms.glRatio
    );
  ...
  connect(inputName, sourceNode, outputName = "default") {
    ...
    // Forward connection to simulation node
    if (this.simulationNode) {
      this.simulationNode.connect(inputName, sourceNode, outputName);
    }
  ...
  disconnect(inputName) {
    ...
    if (connection) {
      ...
      // Forward disconnection to simulation node
      if (this.simulationNode) {
        this.simulationNode.disconnect(inputName);
      }
    }
    return this;
  }
  ...
  dispose() {
    ...
    if (this.simulationNode) this.simulationNode.dispose();
  ...
  getDefaultSimulationShader() {
    return `#version 300 es
      ...
      void main() {
        ...
        // Initialize new particles in a grid pattern if they're at origin
        if (length(pos) < 0.001 && length(vel) < 0.001) {
          pos = glUV; // Start at UV coordinates (0-1 space)
          vel = vec2(0.0);
        }
        
        // Apply mouse force (repulsion/attraction)
        vec2 mousePos = glMouse.xy;
        vec2 toMouse = pos - mousePos;
        float distToMouse = length(toMouse);
        
        // Apply mouse force with configurable radius
        float maxRadius = mouseRadius;
        if (distToMouse < maxRadius && distToMouse > 0.001) {
          float strength = 1.0 - (distToMouse / maxRadius);
          strength = strength * strength; // Quadratic falloff
          
          vec2 forceDir = normalize(toMouse);
          vel += forceDir * mouseForce * strength;
        }
        
        // Spring back to grid position (UV coordinates)
        vec2 restPos = glUV;
        vec2 toRest = restPos - pos;
        vel += toRest * springForce;
        ...
        // Keep particles in bounds (0-1 space)
  ...
  getDefaultVertexShader() {
    return `#version 300 es
      ...
      void main() {
        ...
        // Convert to screen space (-1 to 1) with aspect ratio correction
        vec2 screenPos = particlePos * 2.0 - 1.0;
        screenPos.x /= glRatio; // Correct for aspect ratio
        
        // Apply quad offset (also corrected for aspect ratio)
        vec2 quadOffset = quadVertex * particleSize;
        quadOffset.x /= glRatio; // Keep particles circular regardless of aspect ratio
        ...
        float speed = length(particleVelocity) * 50.0;
        particleColor = vec4(1.0, 1.0 - speed, 1.0 - speed * 0.5, 1.0);
  ...
  getDefaultFragmentShader() {
    return `#version 300 es
      ...
      void main() {
        ...
        float dist = length(particleUV - 0.5) * 2.0;
        float alpha = 1.0 - smoothstep(0.6, 1.0, dist);
        
        // Add some glow
        alpha += (1.0 - smoothstep(0.0, 1.2, dist)) * 0.3;
...
class miniGL {
  ...
  // Factory method for particle system
