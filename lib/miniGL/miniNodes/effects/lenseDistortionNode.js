...
export function LenseDistortionNode(gl, uniforms = {}, nodeOptions = {}) {
  ...
  const shader = gl.shader(frag);
  const shaderB = gl.shader(frag2);
  shaderB.connect(shader, "glTexture");
  ...
  const mini = new MiniNode(gl, distortionNode, {
  ...
  mini.input("uMasked", "uMasked");

  // Map external uniform names to internal shader uniforms
  mini.uniform("intensity", "uIntensity");
  mini.uniform("radius", "uRadius");
  mini.uniform("distortion", "uDistortion");
  mini.uniform("dispersion", "uDispersion");

  // Set up custom connection handling for masked input
  mini
    .onConnect((inputName, sourceNode, outputName) => {
      if (inputName === "uMasked") {
        mini.updateUniform("uUseMasked", true);
      }
    })
    .onDisconnect((inputName) => {
      if (inputName === "uMasked") {
        mini.updateUniform("uUseMasked", false);
      }
    });
