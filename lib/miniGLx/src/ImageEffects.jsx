...
function CosmeticBrand() {
  ...
  // Product placeholder effects
  const setupProductEffect = (canvasRef, brandColor) => {
    return useMiniGL(
      canvasRef,
      (gl) => {
        // Create a beautiful product placeholder
        ...
        void main() {
          ...
          // Product bottle/container shape
          float bottle = 1.0 - smoothstep(0.2, 0.45, dist);
          
          // Animated shimmer
          float time = glTime * 0.5;
          float shimmer = sin(uv.y * 10.0 + time) * 0.5 + 0.5;
          shimmer *= sin(uv.x * 8.0 + time * 0.7) * 0.5 + 0.5;
          ...
          // Product base color
          vec3 baseColor = mix(vec3(0.95), uBrandColor, 0.3);
          
          // Add shimmer and mouse effects
          vec3 finalColor = baseColor + shimmer * 0.2;
          finalColor += mouseEffect * uBrandColor * 0.3;
          finalColor *= bottle;
          
          // Background gradient
          vec3 bgColor = mix(vec3(0.98, 0.96, 0.97), vec3(0.92, 0.88, 0.91), uv.y);
          finalColor = mix(bgColor, finalColor, bottle);
  ...
  const product1Effect = setupProductEffect(product1Ref, [0.95, 0.7, 0.8]);
  const product2Effect = setupProductEffect(product2Ref, [0.9, 0.8, 0.95]);
  const product3Effect = setupProductEffect(product3Ref, [0.85, 0.9, 0.8]);
