/**
 * lenseDistortion node for miniGL
 *
 * Usage:
 *   import lenseDistortion from './miniNodes/effects/lenseDistortion.js';
 *   const node = lenseDistortion(gl, inputNode, { uniforms: { ... } });
 *
 * Behaves like a regular miniGL node (uniforms, updateUniform, etc).
 */
...
export default function lenseDistortion(gl, input, options = {}) {
  // Merge user uniforms with defaults
  const uniforms = Object.assign(
    {
      glTexture: { texture: null }, // will connect below
    },
    options.uniforms || {}
  );
  ...
  // Connect the input node/texture
  gl.connect(input, node, "glTexture");

  // Proxy updateUniform for convenience
  node.setUniform = node.updateUniform.bind(node);
